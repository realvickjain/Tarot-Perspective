 ...
 * 2. Create a project and an OAuth Web Client ID.
 * 3. Add THIS APP'S URL to "Authorized JavaScript Origins".
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (g && GOOGLE_CLIENT_ID && !GOOGLE_CLIENT_ID.includes("YOUR_GOOGLE_CLIENT_ID_HERE")) {
      ...
      console.warn("Perspective: Google Client ID is not configured. Please see App.tsx for setup steps.");
    }
  }, []);

  // Handle rendering of the Google Button when the overlay is shown
  useEffect(() => {
    const g = (window as any).google;
  ...
  const handleGoogleCredentialResponse = (response: any) => {
    ...
    if (payload) {
      ...
      // If we were in the middle of a flow, proceed
  ...
  const handleAnalyzeSpread = async () => {
    ...
    try {
      ...
      const initialPulls = spread.positions.map((pos, idx) => ({
        position: pos,
        card: shuffled[idx]
      }));
  ...
  const isClientConfigured = !GOOGLE_CLIENT_ID.includes("YOUR_GOOGLE_CLIENT_ID_HERE");
        ...
        <nav className="flex items-center gap-6">
          ...
          {user ? (
            ...
            <button 
              ...
              className="bg-white/10 hover:bg-white/20 text-white text-xs font-bold py-2.5 px-5 rounded-xl border border-white/20 transition-all"
            >
              Sign In
            </button>
          )}
        ...
        <main className="w-full max-w-[800px] bg-white rounded-[2.5rem] shadow-2xl p-8 md:p-14 animate-fadeIn transition-all duration-500 min-h-[600px] flex flex-col relative overflow-hidden">
          ...
          {step === AppStep.LANDING ? (
            <div className="space-y-12 animate-fadeIn">
              <section>
                <div className="mb-10 text-center sm:text-left">
                  ...
                  <p className="text-slate-500 mt-3 text-lg font-medium leading-relaxed">
                    A modern mirror for your thoughts, blending timeless symbolism with intelligent reflection.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-2 bg-slate-100/80 p-1.5 rounded-2xl">
                  {CATEGORIES.map((cat) => (
                    <button
                      ...
                      className={`flex-1 min-w-[100px] py-4 px-2 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${
                        category === cat 
              ...
              <section>
                ...
                <div className="relative">
                  <textarea
                    ...
                    placeholder="Briefly share what's on your mind right now..."
                    className="w-full p-6 h-56 bg-slate-50 border border-slate-100 rounded-[2rem] focus:ring-4 focus:ring-indigo-50 focus:border-indigo-100 outline-none transition-all resize-none text-slate-700 placeholder:text-slate-300 font-medium leading-relaxed shadow-inner"
                  />
                  <div className="absolute bottom-6 right-8 text-[10px] font-bold text-slate-300 tracking-widest uppercase">
                    {question.length}/300
                  </div>
                </div>
              </section>
              ...
              <button
                disabled={!category}
                onClick={handleAnalyzeSpread}
                className={`w-full py-6 rounded-2xl font-bold text-base transition-all shadow-xl flex items-center justify-center gap-2 group mt-4 ${
                  category 
            ...
            <div className="animate-fadeIn space-y-12 h-full flex flex-col">
              ...
              <div className="flex-1 flex flex-wrap justify-center items-center gap-6 md:gap-10 py-10">
                {pulls.map((pull, idx) => (
                  <div key={idx} className="flex flex-col items-center">
                     <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-indigo-500 mb-5 bg-indigo-50 px-4 py-1.5 rounded-full border border-indigo-100 shadow-sm">
                       {pull.position.title}
                     </span>
                     <Card 
                       card={pull.card}
              ...
              <div className="mt-auto">
                <button
                  disabled={!allFlipped}
                  onClick={initiateInterpretation}
                  className={`w-full py-6 rounded-2xl font-bold text-base transition-all shadow-xl flex items-center justify-center gap-2 ${
                    allFlipped 
                    ? 'bg-indigo-600 text-white hover:bg-indigo-700 animate-pulse shadow-indigo-100' 
                ...
                {!allFlipped && (
                  <p className="text-center text-slate-400 text-xs mt-4 italic font-bold tracking-wide">Please reveal all cards to proceed.</p>
                )}
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center py-20 animate-fadeIn">
              ...
              <div className="mt-12 flex items-center gap-2 px-6 py-3 bg-indigo-50 rounded-2xl border border-indigo-100">
                ...
                <span className="text-xs font-bold text-indigo-600 uppercase tracking-widest">
                  {emailStatus === 'sending' ? 'Delivering copy to your inbox...' : 'Email delivery prepared'}
                </span>
              </div>
            </div>
            ...
            <div className="animate-fadeIn space-y-10 flex-1 flex flex-col">
              <div className="flex justify-between items-start border-b border-slate-100 pb-12">
                <h2 className="text-4xl font-extrabold text-slate-900 leading-[1.15] italic tracking-tight flex-1">
              ...
              <div className="mt-auto pt-12">
                <button
                  onClick={reset}
                  className="w-full py-6 border-2 border-slate-100 text-slate-400 rounded-2xl font-bold text-sm hover:bg-slate-50 hover:text-slate-600 hover:border-slate-200 transition-all"
                >
          ...
          <div className="bg-white w-full max-w-[440px] rounded-[2.5rem] shadow-[0_32px_128px_-16px_rgba(30,27,75,0.4)] overflow-hidden">
            <div className="p-10 flex flex-col items-center text-center">
              ...
              <p className="text-slate-500 mt-4 text-sm font-medium leading-relaxed">
                Sign in to archive this reading and receive a detailed breakdown in your inbox.
              </p>
              ...
              <div className="w-full mt-10">
                {isClientConfigured ? (
                  ...
                  <div className="p-6 bg-amber-50 rounded-2xl border border-amber-100 text-left">
                    <p className="text-amber-800 text-xs font-bold mb-2">Configuration Required</p>
                    <p className="text-amber-700 text-[11px] leading-relaxed">
                      To enable Google Login, you must register your Client ID in <code>App.tsx</code>. 
                      Follow the instructions provided in the solution steps.
                    </p>
                  </div>
                )}
              </div>
              ...
              <button 
                ...
                className="mt-6 text-slate-400 text-[11px] font-bold uppercase tracking-widest hover:text-indigo-600 transition-colors"
              >
                Continue as Guest
              </button>
            </div>
            ...
            <div className="bg-slate-50 p-6 text-center border-t border-slate-100">
              <p className="text-[10px] text-slate-400 leading-relaxed px-4">
                By signing in, you agree to our Terms of Service and Privacy Policy. We only use your email to send copies of your readings.
              </p>
            </div>
          </div>
        </div>
